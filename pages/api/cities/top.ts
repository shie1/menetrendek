...
export default async function handler(
    ...
    const topCities = await query(`SELECT c.name, co.name as county, co.id as county_id, c.id, count(s.city) as stops FROM cities AS c LEFT OUTER JOIN stops AS s ON s.city = c.id LEFT OUTER JOIN counties as co ON c.county=co.id GROUP BY c.id ORDER BY stops DESC LIMIT 15;`,)
