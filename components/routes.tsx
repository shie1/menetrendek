...
export const RouteSummary = memo(({ item, }: { item: any, }) => {
    ...
    return (<Stack spacing={0}>
        ...
        <Group position="center" spacing='sm'>
            ...
            {item.totalFare > 0 ? <Text size="sm">{currency.format(calcDisc(item.totalFare, cookies["discount-percentage"]))}</Text> : <></>}
            <Text size="sm">{item.ossztav}</Text>
        </Group>
    </Stack>)
})

export const RouteExposition = memo(({ details, iconSize, withInfoButton }: { details: any, iconSize?: number, withInfoButton?: boolean }) => {
    ...
    return (<Timeline active={99}>
        {!details ? <></> : Object.keys(details.results).map((i: any) => {
            ...
            return (<Timeline.Item bulletSize={25} key={i} title={dataItem.allomas} bullet={<ActionBullet type={cookies["action-timeline-type"]} size={iconSize} prev={details.results[i - 1]} muvelet={dataItem.muvelet} network={dataItem.network} />} lineVariant={dataItem.muvelet === "átszállás" ? "dashed" : "solid"}>
                    ...
                    {!dataItem.jaratinfo ? <></> : <>
                        ...
                        <Group spacing={10}>
                            {!dataItem.jaratinfo.fare || dataItem.jaratinfo.fare < 0 ? <></> :
                                <Text size="sm">{currency.format(calcDisc(dataItem.jaratinfo.fare, cookies["discount-percentage"]))}</Text>}
